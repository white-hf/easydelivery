 ...
 * 2) 可插拔：委托给 DeliveryFocusManager 做距离/集合计算；委托给 InfoPillProximityController 做显隐决策。
 * 3) 可测性：核心入口 onLocation(...) 为纯函数风格（无副作用除回调），便于单测。
 * Phase 3: exposes profile & ETA hooks; centralizes nearby collection with DeliveryFocusManager when available.
 */
public final class ProximityCoordinator {
    ...
    @NonNull private final DeliveryFocusManager focusMgr;
    @NonNull private final InfoPillProximityController proximityController;
    @Nullable private Listener listener;
    @Nullable private Boostable boostable;
    ...
    @Nullable private String lastTargetKey = null; // last visible target key
    ...
    public void onDeliveryCompleted(@Nullable DeliveryInfo delivered) {
        String key = "null";
        if (delivered != null) {
            if (delivered.getOrderId() != null) key = "ID-" + delivered.getOrderId();
            else if (delivered.getOrderSn() != null) key = "SN-" + delivered.getOrderSn();
            else if (delivered.getRouteNumber() != null) key = "ROUTE-" + delivered.getRouteNumber();
        }
        logD("onDeliveryCompleted invoked for " + key);
        try { proximityController.onDeliveryCompleted(delivered); } catch (Throwable ignore) {}
    }

    /** Convenience: adapt SmartLocationManager to Boostable. */
    public static Boostable asBoostable(@NonNull SmartLocationManager mgr){
        return durationMs -> {
            try { mgr.requestBoost(durationMs); } catch (Throwable ignore) {}
        };
    }
    ...
    public void setBoostable(@Nullable Boostable b) { this.boostable = b; }

    public void setNearbyRadiusMeters(float r) { this.nearbyRadiusMeters = Math.max(0f, r); }
    public void setNearbyLimit(int k) { this.nearbyLimit = Math.max(1, k); }

    // Convenience getters/setters for external panels/dev tools
    public float getNearbyRadiusMeters() { return nearbyRadiusMeters; }
    public int getNearbyLimit() { return nearbyLimit; }
    ...
    public InfoPillProximityController.ProximityProfile getProximityProfile() {
        try { return proximityController.getProfile(); } catch (Throwable t) {
    ...
    public void onLocation(@NonNull Location location,
        ...
        if (d.requestBoost && boostable != null) {
            try { boostable.requestBoost(d.boostDurationMs); } catch (Throwable ignore) {}
        }
        ...
        if (d.show || d.update) {
            ...
            // 拿 top-K 作为近邻候选（DeliveryInfo 列表），再在半径内收敛
    ...
    private static String buildKey(@NonNull DeliveryInfo di) {
        if (di.getOrderId() != null) return "ID-" + di.getOrderId();
        if (di.getOrderSn() != null) return "SN-" + di.getOrderSn();
        if (di.getRouteNumber() != null) return "ROUTE-" + di.getRouteNumber();
    ...
    private boolean isDuplicateUpdate(@NonNull DeliveryInfo target, float distance, int nearbySize) {
        if (!lastVisible) return false; // previously hidden -> not dup
        final String key = buildKey(target);
        if (lastTargetKey == null || !key.equals(lastTargetKey)) return false;
        if (Float.isNaN(distance) || Float.isNaN(lastDistanceMeters)) return false;
        if (Math.abs(distance - lastDistanceMeters) > UPDATE_EPSILON_M) return false;
    ...
    private void dispatchHide() {
        logD("dispatchHide");
        Listener l = this.listener; if (l == null) { resetState(); return; }
        try { l.onHide(); } catch (Throwable ignore) {}
        resetState();
    }
}
